"""Token-protected, one-time logical restore for the MedPark One portal.

Only the fixed tables and columns below can be written. The endpoint is disabled
unless ``PORTAL_RESTORE_TOKEN`` exists and is at least 32 characters long.
Calendar integration data is accepted only as an empty, verified exclusion.
"""
import hashlib
import hmac
import json
...
SNAPSHOT_TABLES = {**TABLES, **EXCLUDED_TABLES}
INSERT_ORDER = list(TABLES)
DELETE_ORDER = [
    "portal_sessions", "user_quick_links", "portal_custom_menu_items",
    "portal_menu_order", "portal_menu_labels", "audit_logs",
    "portal_app_migrations", "users",
]
...
def migration_enabled():
    return len(os.environ.get("PORTAL_RESTORE_TOKEN", "")) >= 32


def authorize(value):
    expected = os.environ.get("PORTAL_RESTORE_TOKEN", "")
...
def validate_snapshot(snapshot):
    ...
    if not isinstance(payload_tables, dict) or set(payload_tables) != set(SNAPSHOT_TABLES):
    ...
    for table in EXCLUDED_TABLES:
        if payload_tables[table]["row_count"] != 0:
            raise MigrationError(f"{table}은 복원 대상에서 제외되어야 합니다.")
    return {table: payload_tables[table] for table in TABLES}
...
def _adapt_value(table, column, value):
    if table == "audit_logs" and column == "metadata" and isinstance(value, (dict, list)):
        return core.psycopg2.extras.Json(value)
    return value
...
def import_snapshot(snapshot):
    ...
    return {
        ...
        "api_integrations_excluded": True,
    }
