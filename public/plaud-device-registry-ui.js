(() => {
  if (window.__MEDPARK_PLAUD_EASY_REGISTRY_V4__) return;
  window.__MEDPARK_PLAUD_EASY_REGISTRY_V4__ = true;

  const PANEL_ID = "plaudDeviceDirectRegistry";
  const STYLE_ID = "plaudEasyRegistryV4Style";
  let plaudState = { items: [], users: [], plaud_configured: false };
  let androidState = { items: [], users: [] };
  let portalUsers = [];
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .per-assignee-note{margin:0 0 12px;padding:10px 12px;border:1px solid #cfe3dc;border-radius:10px;background:#eef8f4;color:#47685f;font-size:8px;line-height:1.6}.per-assignee-note b{color:#075f51}.per-user-count{margin-left:6px;font-weight:900;color:#087563}
  ...
  function populateUserOptions(panel) {
    const entries = portalUsers.map((u) => ({
      value: String(u.id),
      label: `${u.name || u.username || "임직원"}${u.department ? ` · ${u.department}` : ""}${u.username ? ` · ${u.username}` : ""}`,
    }));
    replaceOptions(panel?.querySelector("#perAndroidUser"), entries, entries.length ? "임직원 선택" : "선택 가능한 활성 임직원 없음");
    replaceOptions(panel?.querySelector("#perPlaudUser"), entries, entries.length ? "임직원 선택" : "선택 가능한 활성 임직원 없음");
    const count = panel?.querySelector("#perUserCount");
    if (count) count.textContent = entries.length ? `활성 임직원 ${entries.length}명` : "활성 임직원 0명";
    if (!entries.length) {
      msg(panel, "#perAndroidMessage", "활성 임직원 목록을 불러오지 못했습니다. 포털 관리 → 임직원 관리에서 계정 상태를 확인해 주세요.", "error");
    }
  }

  function selectedUserById(id) {
    return portalUsers.find((u) => String(u.id) === String(id));
  }
  ...
  async function loadAll(panel) {
    const [adminUsersResult, meResult, androidResult, plaudResult] = await Promise.allSettled([
      api("/api/admin/users"),
      api("/api/auth/me"),
    ...
    androidState = androidResult.status === "fulfilled" ? normalizeRegistry(androidResult.value) : normalizeRegistry({});
    plaudState = plaudResult.status === "fulfilled" ? normalizeRegistry(plaudResult.value) : normalizeRegistry({});

    const adminUsers = adminUsersResult.status === "fulfilled" && Array.isArray(adminUsersResult.value?.users)
      ? adminUsersResult.value.users.filter((u) => u?.status === "active") : [];
    const me = meResult.status === "fulfilled" && meResult.value?.user?.status === "active" ? [meResult.value.user] : [];
    portalUsers = mergeUsers(adminUsers, me, androidState.users, plaudState.users)
      .filter((u) => !u.status || u.status === "active")
      .map((u) => ({ ...u, suggested_partner_user_id: u.suggested_partner_user_id || partnerUserId(u) }));

    if (adminUsersResult.status === "rejected" && me.length) {
      msg(panel, "#perAndroidMessage", "전체 임직원 목록을 불러오지 못해 현재 로그인 사용자만 표시합니다.", "warn");
    }
    if (androidResult.status === "rejected") msg(panel, "#perAndroidMessage", `Android Registry: ${androidResult.reason.message}`, "error");
    if (plaudResult.status === "rejected") msg(panel, "#perPlaudMessage", `PLAUD Registry: ${plaudResult.reason.message}`, "error");
  ...
  async function registerPlaud(panel, button) {
    ...
    const partnerUser = panel.querySelector("#perPartnerUser")?.value.trim() || user.suggested_partner_user_id || partnerUserId(user);
  ...
  document.addEventListener("change", (event) => {
    ...
    if (event.target.id === "perPlaudUser") {
      ...
      if (partner) partner.value = user?.suggested_partner_user_id || (user ? partnerUserId(user) : "");
    }
  }, true);
  ...
  function attach() {
    ...
    if (!panel || panel.dataset.easyV4Loaded === "1") return;
    panel.dataset.easyV4Loaded = "1";
    loadAll(panel);
  }
