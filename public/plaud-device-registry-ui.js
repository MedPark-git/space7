(() => {
  ...
  function getHost(target = null) {
    return target?.closest?.(ROOT_SELECTOR) || document.querySelector(`#${DIRECT_ID}`) || document.querySelector(ROOT_SELECTOR);
  }
  ...
  function selectedUser(host) {
    const select = host?.querySelector("#pdrAssignedUser");
    return select ? state.users.find((user) => String(user.id) === select.value) || null : null;
  }

  function syncPartnerId(host) {
    const user = selectedUser(host);
    const input = host?.querySelector("#pdrPartnerUserId");
  ...
  function renderRegistry(host) {
    ...
    if (head && !head.querySelector(".pdr-title-wrap")) {
      ...
      if (h3) {
        ...
        h3.replaceWith(wrap);
        wrap.innerHTML = `${iconSvg}<h3>${escapeHtml(title)}</h3>`;
      }
    }
    ...
    toolbar.innerHTML = `<div class="pdr-status-line"><i class="pdr-status-dot ${state.plaud_configured ? "ready" : ""}"></i><span>${state.plaud_configured ? "PLAUD Developer API 연결 준비됨" : "PLAUD API 인증정보 확인 필요 · Registry 등록은 가능"}</span></div>${state.can_manage === false ? "" : '<button type="button" id="pdrAddButton" class="pdr-add-button" data-pdr-open="1"><span>＋</span> 기기 등록</button>'}`;
    host.querySelector("#pdrRegisterFormWrap")?.remove();
    toolbar.insertAdjacentHTML("afterend", renderForm());
    const body = host.querySelector(".peg-reg-table tbody");
    if (body) body.innerHTML = state.items.length ? state.items.map(rowMarkup).join("") : `<tr><td colspan="10"><div class="pdr-empty-note"><strong>등록된 PLAUD 기기가 없습니다.</strong><span>${state.can_manage === false ? "관리자가 기기를 등록하면 표시됩니다." : "오른쪽의 '기기 등록' 버튼을 눌러 실제 기기를 등록해 주세요."}</span></div></td></tr>`;
  ...
  function showFormMessage(host, message, error = false) {
    const node = host?.querySelector("#pdrFormMessage");
    if (!node) return;
    node.textContent = message;
    node.className = `pdr-message show${error ? " error" : ""}`;
  }
  ...
  async function loadRegistry(host) {
    ...
    try {
      ...
      if (host.isConnected) renderRegistry(host);
    } catch (error) {
      const body = host.querySelector(".peg-reg-table tbody");
      if (body) body.innerHTML = `<tr><td colspan="10"><div class="pdr-empty-note"><strong>기기 Registry API 연결을 확인해 주세요.</strong><span>${escapeHtml(error.message)}</span></div></td></tr>`;
    } finally {
      loading = false;
    }
  }

  async function submitRegistration(host, button) {
    ...
    if (!payload.serial_number || !payload.assigned_user_id) {
      showFormMessage(host, "Serial Number와 할당 사용자를 입력해 주세요.", true);
      return;
    }
    ...
    try {
      ...
      showFormMessage(host, error.message, true);
    } finally {
      button.disabled = false;
    }
  }

  async function handleDeviceAction(host, button) {
    ...
    if (action === "bind" && !confirm(`PLAUD Cloud에 이 기기를 바인딩하시겠습니까?\n\n기기: ${item.device_name}\nSN: ${item.serial_number}\n사용자: ${item.assigned_user_name}\nPartner User ID: ${item.partner_user_id}\n\n바인딩 후 Android 앱에서 BLE Bind/Handshake를 완료해야 합니다.`)) return;
    if (action === "unbind" && !confirm("PLAUD Cloud 바인딩을 해제하시겠습니까?\nAndroid 앱에서도 depair가 필요합니다.")) return;
    button.disabled = true;
    try {
  ...
  // Persistent delegated events: these survive every SPA re-render and toolbar replacement.
  document.addEventListener("click", async (event) => {
    const addButton = event.target.closest("#pdrAddButton,[data-pdr-open='1']");
    if (addButton) {
      event.preventDefault();
      event.stopPropagation();
      const host = getHost(addButton);
      if (!host) return;
      let form = host.querySelector("#pdrRegisterFormWrap");
      if (!form) {
        renderRegistry(host);
        form = host.querySelector("#pdrRegisterFormWrap");
      }
      if (form) {
        partnerTouched = false;
    ...
    const cancel = event.target.closest("#pdrCancel");
    if (cancel) {
      event.preventDefault();
      getHost(cancel)?.querySelector("#pdrRegisterFormWrap")?.classList.remove("open");
      return;
    }

    const submit = event.target.closest("#pdrSubmit");
    if (submit) {
      event.preventDefault();
      await submitRegistration(getHost(submit), submit);
      return;
    }
    ...
    if (actionButton) {
      event.preventDefault();
      await handleDeviceAction(getHost(actionButton), actionButton);
    }
  }, true);

  document.addEventListener("change", (event) => {
    if (event.target.matches("#pdrAssignedUser")) syncPartnerId(getHost(event.target));
  }, true);

  document.addEventListener("input", (event) => {
    const host = getHost(event.target);
    if (event.target.matches("#pdrPartnerUserId")) partnerTouched = true;
    if (event.target.matches("#pdrSerial")) {
  ...
  function attach() {
    ensureStyle();
    if (!isPlaudPage()) return;
    const host = document.querySelector(`#${DIRECT_ID}`) || document.querySelector(ROOT_SELECTOR) || createDirectHost();
    if (!host) return;
    if (host.dataset.registryLoaded === "1") return;
    host.dataset.registryLoaded = "1";
    loadRegistry(host);
  }
