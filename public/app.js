...
const renderCalendarDays = (events = [], selectedKey = "") => {
  ...
  const days = Array.from({ length: 42 }, (_, index) => {
    ...
    return `<button type="button" class="calendar-date ${muted ? "muted-day" : ""} ${key === todayKey ? "today-day" : ""} ${key === selectedKey ? "selected-day" : ""}" data-calendar-date="${key}" aria-label="${date.getMonth() + 1}월 ${date.getDate()}일 일정 ${dayEvents.length}건">${date.getDate()}${dayEvents.length ? `<small class="mobile-event-count" aria-hidden="true">${dayEvents.length}</small>` : ""}${dayEvents.slice(0,2).map((event) => `<span class="event-dot" style="--event-color:${escapeHtml(event.calendar_color || "#0a9b7e")};--event-foreground:${escapeHtml(event.calendar_foreground || "#ffffff")}" title="${escapeHtml(event.calendar_name || "Google Calendar")}">${escapeHtml(event.title)}</span>`).join("")}${dayEvents.length > 2 ? `<small class="more-events">+${dayEvents.length - 2}</small>` : ""}</button>`;
  }).join("");
  return heads + days;
};
